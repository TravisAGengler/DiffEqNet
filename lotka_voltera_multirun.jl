...
# In the case of the lotka_voltera problem, this means that the first two elements are the u0 and the next 4 are the params
# Since lotka_voltera has two variables (x,y) and 4 params (α, β, δ, γ)
...
function train_models_with_params(all_init_u0_params)
    # These are the true parameters we are attempting to learn
